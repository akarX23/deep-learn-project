"""Standalone Kafka worker runtime for Teaching Agent event processing."""
...
from project.schemas import TeachingCompletionEvent, WorkerRuntimeState
from project.topics import get_teaching_topic_names
    ...
    for records in batches.values():
        for record in records:
            ...
            logger.info(
                ...
                request_id, getattr(record, "topic", "teaching"),
            )
            try:
...
class TeachingWorker:
    ...
    def __init__(
        ...
        handler_factory: Callable[[], TeachingRequestEventHandler] = TeachingRequestEventHandler,
    ) -> None:
        self._config = config
    ...
    def start(self) -> None:
        ...
        consumer_subscribe_teaching(self._consumer)

        topic_check = check_required_topics(self._consumer, get_teaching_topic_names())
        ...
        self._thread = threading.Thread(
            ...
            args=(handler,),
            daemon=True,
            name="teaching-kafka-worker-loop",
        )
        self._thread.start()
