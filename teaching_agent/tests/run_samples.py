...
for entry in TOPICS:
    ...
    for mode in MODES:
        ...
        print(f"status={status}  tokens={tokens}  time={elapsed:.2f}s  diagram={diagram}  -> {out_path.name}")
...
print("Summary")
print(f"{'='*60}")
print(f"{'Topic':<35} {'Mode':<14} {'Status':<8} {'Model':<32} {'Tokens':>6} {'Time(s)':>8}  {'Diagram'}")
print("-" * 115)
for r in results:
    print(
        f"{r['topic']:<35} {r['mode']:<14} {r['status']:<8} {r['model']:<32} {r['tokens_used']:>6} {r['time_s']:>8.2f}  {'yes' if r['has_diagram'] else 'no'}"
    )
