...
Strategy
--------
* All LLM calls are monkeypatched via ``patch("planner_agent.agent.call_llm")``.
...
from planner_agent.classifier import (
...
def test_planner_request_event_rejects_empty_prompt() -> None:
    with pytest.raises(Exception):
        PlannerRequestEvent(user_prompt="", sid="s1")
...
def test_assess_learner_level_naive(dummy_llm_config: LLMConfig) -> None:
    with patch("planner_agent.classifier.call_llm_json") as m:
...
def test_assess_learner_level_fallback_on_error(dummy_llm_config: LLMConfig) -> None:
    with patch(
        "planner_agent.classifier.call_llm_json",
...
def test_pipeline_teaching_only_no_files(
    ...
    mock_teaching_response["user_level"] = "naive"

    producer = MockProducer()
    agent = _make_agent(producer)

    with (
        patch("planner_agent.agent.call_llm", return_value=_llm_level_response("naive")),
        patch(
            "planner_agent.agent.interrupt",
    ...
    assert "planner-response" in producer.topics()
    assert "rag" not in producer.topics()
    assert producer.last("planner-response")["status"] in {"complete", "partial"}
...
def test_pipeline_quiz_intent_dispatches_quiz(
    ...
    assert "quiz" in producer.topics()
    assert "planner-response" in producer.topics()


def test_pipeline_injection_blocked() -> None:
    """Injection query → clarify_and_end → failed planner-response; no agents dispatched."""
    producer = MockProducer()
    agent = _make_agent(producer)
    ...
    assert "quiz" not in topics
    assert "planner-response" in topics
    assert producer.last("planner-response")["status"] == "failed"
...
def test_pipeline_pre_provided_levels_skip_llm(
    ...
    with (
        patch("planner_agent.agent.call_llm", side_effect=lambda *a, **k: llm_calls.append(a) or ""),
        patch(
            "planner_agent.agent.interrupt",
            side_effect=_make_interrupt_mock(teaching_payload=mock_teaching_response),
        ),
...
def test_pipeline_keyed_produce(
    ...
    with (
        patch("planner_agent.agent.call_llm", return_value=_llm_level_response("intermediate")),
        patch(
            "planner_agent.agent.interrupt",
            side_effect=_make_interrupt_mock(teaching_payload=mock_teaching_response),
        ),
...
def test_pipeline_llm_failure_routes_to_clarify() -> None:
    ...
    with (
        patch("planner_agent.agent.call_llm", side_effect=RuntimeError("LLM down")),
        patch(
            "planner_agent.agent.interrupt",
...
def test_workflow_complete_event_valid() -> None:
    evt = WorkflowCompleteEvent(
        request_id="abc123",
        sid="sess-1",
        teaching_materials={"naive": "Some content"},
        status="complete",
    )
    assert evt.status == "complete"


def test_workflow_complete_event_invalid_status() -> None:
    with pytest.raises(Exception):
        WorkflowCompleteEvent(request_id="abc123", sid="s", status="unknown")
